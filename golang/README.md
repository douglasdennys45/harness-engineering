# Harness de Engenharia — Go

Este diretório contém um **harness de desenvolvimento orientado a specs** para serviços Go usando Claude Code. Ele define um pipeline completo — da ideia ao merge — em que cada fase produz um artefato versionado, e a implementação só avança com aprovação explícita (do usuário nas fases de planejamento, dos agents de review/QA na fase de execução).
...
| `skills/` | Boas práticas consultadas pelos commands e agents (Go, Fiber v3, PostgreSQL, NATS, testes, commits, QA, review, bugfix, brainstorming, PRD) |
...
Brainstorm técnico sobre o PRD: lê o codebase, propõe 2-5 abordagens arquiteturais validadas contra as skills de domínio (Go, Fiber v3, Postgres, NATS) e as rules do projeto, avalia reusar vs construir, e define a estratégia de testes desde o início. Só gera a spec após o usuário confirmar a direção técnica.
...
Lê PRD + Tech Spec + codebase, mapeia os artefatos por camada (domain → application → infrastructure → migrations → composição FX) e propõe 2-3 abordagens de decomposição. **Mostra a lista high-level e aguarda aprovação explícita** antes de gerar qualquer arquivo.
...
3. **Implementação:** segue `rules/architecture.md` e consulta as skills conforme o contexto (Fiber para controllers, Postgres para repositórios, NATS para mensageria, etc.). Testes escritos junto com o código.
4. **Validação (gate):** `go build ./...`, `go vet ./...`, `go test ./... -race -count=1` — 100% de sucesso é pré-condição.
...
|---|---|---|---|
| `task-reviewer` | Após cada tarefa commitada | Revisa o diff contra Go idioms, Clean Architecture e a stack; roda as verificações automatizadas; grava `tasks/reviews/[num]_task_review.md` | `APPROVED` \| `APPROVED WITH OBSERVATIONS` \| `CHANGES REQUESTED` |
| `qa-validator` | Quando todas as tasks estão APPROVED | Pipeline de QA completo: análise estática, testes com race detector, mutation testing, E2E de API, contrato Swagger, auditoria de código e infra. **Tolerância zero:** qualquer bug = REPROVADO | `APROVADO` \| `REPROVADO` |
...
| Skill | Quando |
|---|---|
| `go-best-practices` | Qualquer código Go |
| `go-testing-best-practices` | Testes (testify, gomock, table-driven, race) |
| `testcontainers-go` | Testes de integração com Postgres/NATS reais |
| `fiber-v3-best-practices` | Controllers, rotas, middlewares HTTP |
...
- **Go 1.26** em monorepo (`go.work`) com N apps independentes (`apps/<app>/`) + bibliotecas compartilhadas (`pkg/`)
- **Clean Architecture** por app: `domain/` (entidades + interfaces) → `application/` (use cases) → `infrastructure/` (controllers, repos, publishers, subscribers) → `cmd/` (composição FX)
- **Fiber v3** (HTTP), **PostgreSQL 18** (SQL manual, sem ORM), **NATS JetStream 2.12** (mensageria entre apps), **uber-go/fx** (DI), **log/slog** (logging estruturado)
- 1 use case = 1 interface = 1 arquivo, com método único `Perform`
...
3. Tarefa só está completa com testes 100% verdes (`-race -count=1`) **e** review APPROVED.
