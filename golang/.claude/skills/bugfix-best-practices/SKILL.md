...
description: Aplica melhores práticas de Bugfix para correção sistemática de bugs documentados pelo QA em serviços Go (Golang). Cobre leitura do arquivo `.cognup/specs/[feature]/report/bugs.md`, análise de causa raiz (nunca sintomas), planejamento por bug, implementação da correção conforme Clean Architecture (`.claude/rules/architecture.md`) e stack do projeto (Fiber v3, PostgreSQL 18, NATS JetStream, uber-go/fx, slog), criação de testes de regressão (unitário com testify/gomock, integração com testcontainers-go, E2E de API), validação completa (`go build`, `go vet`, `go test -race -count=1`, `golangci-lint`), atualização do bugs.md com status das correções, relatório final, commits no padrão Conventional Commits e ciclo de revisão (task-reviewer até APPROVED, depois qa-validator até APROVADO). Use sempre que houver bugs documentados em bugs.md para corrigir, quando o QA reprovar uma feature, ao corrigir regressões, ou ao criar testes de regressão para bugs encontrados.
---

# Bugfix Best Practices — Correção Sistemática de Bugs Go

Skill que aplica o processo metódico de **Bugfix** sobre bugs documentados pelo QA (`qa-best-practices`) em serviços Go (Golang). Combina análise de causa raiz, correção conforme os padrões do projeto, testes de regressão, validação completa, rastreabilidade no `bugs.md` e o ciclo de revisão até aprovação.

Combina com [[go-best-practices]] (padrões de código idiomático Go) e [[go-testing-best-practices]] (testes de regressão), e — conforme o componente afetado — com [[fiber-v3-best-practices]] (HTTP), [[postgres-best-practices]] (banco), [[nats-best-practices]] (mensageria) e [[testcontainers-go]] (integração). Commits seguem [[conventional-commit]]. A arquitetura de referência é `.claude/rules/architecture.md` (monorepo com Fiber v3, PostgreSQL 18, NATS JetStream e uber-go/fx).
...
- **Nada finaliza com teste falhando** — `go test ./... -race -count=1` com 100% de sucesso é pré-condição
...
- Carregar [[go-best-practices]] (sempre) e as skills de domínio conforme os componentes afetados: [[fiber-v3-best-practices]] para bugs em controllers/handlers, [[postgres-best-practices]] para bugs em repositórios/queries/migrations, [[nats-best-practices]] para bugs em publishers/subscribers
...
| Dado não persiste | Transaction sem commit, `RETURNING` ausente, rollback silencioso |
| Evento não chega ao consumer | Publish antes do commit, subject errado, falta de `Nats-Msg-Id` |
| Teste flakey | Race condition, dependência de ordem, estado compartilhado entre testes |
| Panic intermitente | Nil pointer em caminho de erro não coberto, goroutine sem recover na boundary |
...
3. **Implementar a correção na causa raiz** — conforme os padrões do projeto (camadas, construtores, error handling com `%w`, eventos pós-commit)
4. **Verificar compilação** — `go build ./...` após cada correção
...
- **Cubram edge cases relacionados** — variações do mesmo problema (nil, vazio, limites, concorrência)

> Referência: [[go-testing-best-practices]] (table-driven, testify, gomock) e [[testcontainers-go]] (serviços reais).

| Tipo | Quando Usar | Ferramentas |
|------|-------------|-------------|
| Teste unitário | Bug em lógica isolada de função/método/use case | table-driven + testify + gomock |
| Teste de integração | Bug na comunicação entre módulos ou com infra (repository, publisher/subscriber) | testcontainers-go (PostgreSQL/NATS reais) |
| Teste E2E | Bug visível no fluxo completo da API | `app.Test()` do Fiber / curl contra a API |

Nomeie os testes de forma rastreável ao bug (ex.: `TestCreateOrder_DuplicateEmailReturnsConflict_BUG03` ou subtest `t.Run("BUG-03: duplicate email returns 409", ...)`).
...
golangci-lint run ./...   # se configurado no projeto
...
- `go build ./...`: SEM ERROS
- `go vet ./...`: LIMPO
- `go test ./... -race -count=1`: TODOS PASSANDO
- `golangci-lint run`: [OK / N/A]
```
...
7. **Validação completa antes do commit** — build, vet, test -race, lint, gofmt
...
|---|---|
| Silenciar o sintoma (`if err != nil { return nil }`) | Corrigir a causa raiz do erro |
| `recover()` para esconder panic | Eliminar o nil pointer / estado inválido que causa o panic |
...
| Marcar "Corrigido" sem rodar a suite completa | `go test ./... -race -count=1` a 100% antes de atualizar o bugs.md |
...
| Retry/sleep para "resolver" teste flakey | Corrigir a race condition ou o estado compartilhado |
...
go test -race -run TestCreateOrder ./apps/<app>/internal/application/usecase/order/
...
# Cobertura dos pacotes corrigidos
go test -race -coverprofile=coverage.out ./apps/<app>/...
...
- Causas raiz recorrentes (ex.: erro de domínio não registrado no ErrorMapper, publish dentro de transaction)
