...
- Quais skills complementares carregar (next-best-practices, nextjs-clean-architecture, shadcn, frontend-design, responsive-design, web-design-guidelines, webapp-testing, playwright-generate-test)
...
5. **Changelog e versionamento somente pós-aprovação**: as Etapas 11 (Changelog) e 12 (SemVer no `package.json`) só executam com status APROVADO — use as skills `changelog-automation` e `conventional-commit` como referência; não crie tags git nem commits (isso é do `run-task.md`). Se REPROVADO, pule ambas e documente os bugs.
6. **Padrões do projeto**: valide contra as skills `nextjs-clean-architecture` e `next-best-practices`, incluindo a regra de integração via SSR — chamada direta do client ao backend é bug. Stack: Next.js (App Router), React Server Components, TypeScript, Tailwind, shadcn/ui. Ambiente: localhost.
...
- Padrões de resposta de API e formatos de erro observados no network
- Estrutura de navegação, rotas e padrões de validação de formulários
- Testes flakey conhecidos ou problemas intermitentes
- Comportamentos específicos de breakpoint/browser observados durante os testes
