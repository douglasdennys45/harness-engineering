...
description: Aplica melhores práticas de Bugfix para correção sistemática de bugs documentados pelo QA em aplicações frontend Angular com SSR. Cobre leitura do arquivo `.cognup/specs/[feature]/report/bugs.md`, análise de causa raiz (nunca sintomas), planejamento por bug, implementação da correção conforme os padrões do projeto (angular-clean-architecture, angular-best-practices, regra BFF), criação de testes de regressão (unitário com Vitest/Angular Testing Library, integração com MSW, E2E com Playwright), validação visual no browser (webapp-testing), validação completa (typecheck, lint, test, build via Nx), atualização do bugs.md com status das correções, relatório final, commits no padrão Conventional Commits e ciclo de revisão (task-reviewer até APPROVED, depois qa-validator até APROVADO). Use sempre que houver bugs documentados em bugs.md para corrigir, quando o QA reprovar uma feature, ao corrigir regressões, ou ao criar testes de regressão para bugs encontrados.
---

# Bugfix Best Practices — Correção Sistemática de Bugs Frontend Angular

Skill que aplica o processo metódico de **Bugfix** sobre bugs documentados pelo QA (`qa-best-practices`) em aplicações frontend Angular com SSR. Combina análise de causa raiz, correção conforme os padrões do projeto, testes de regressão, validação completa, rastreabilidade no `bugs.md` e o ciclo de revisão até aprovação.

Combina com [[angular-best-practices]] (padrões Angular/SSR) e [[angular-clean-architecture]] (arquitetura, camadas e federação) — sempre —, e conforme o componente afetado com [[angular-material]] (componentes UI), [[frontend-design]] (bugs visuais), [[responsive-design]] (bugs de layout/responsividade), [[web-design-guidelines]] (conformidade de interface), [[webapp-testing]] (validação no browser) e [[playwright-generate-test]] (testes de regressão E2E). Commits seguem [[conventional-commit]].
...
- Ler a TechSpec para entender as decisões técnicas relevantes (render modes, contratos de API/BFF, componentes)
- Consultar [[angular-clean-architecture]] para garantir conformidade nas correções
- Carregar [[angular-best-practices]] (sempre) e as skills de escopo conforme os componentes afetados: [[angular-material]] para bugs em componentes UI, [[responsive-design]] para bugs de layout/breakpoint, [[frontend-design]]/[[web-design-guidelines]] para bugs visuais e de UX
...
Componente Afetado: [camada + arquivo(s) + projeto Nx]
...
|---|---|
| Erro NG0500/NG0501 (hydration mismatch) no console | Render não-determinístico (data/random/browser API no render), HTML inválido, condição baseada em localStorage |
| Dado não aparece na tela | Repository com fetch manual (sem transfer cache), contrato de API divergente, signal não atualizado, render mode errado |
| Tela pisca conteúdo deslogado antes do logado | Auth via localStorage (invisível ao SSR) — mover para cookies |
| Formulário "não envia" | Mutação sem tratamento de erro no BFF, estado de erro não exibido, submit sem estado pending |
| Componente não atualiza (zoneless) | Estado mutado fora de signal (callback de terceiro, setTimeout cru) — escrever em signal |
| Widget federado quebra a página | Falta de `@defer` + fallback/`@error` no consumo do remote |
| Layout quebrado no mobile | Falta de estratégia mobile-first, largura fixa, ausência de wrap/minmax |
| Erro só em produção | Diferença dev/prod (em dev o remote carrega só no client; prerender só no build) não coberta pelo build local |
...
3. **Implementar a correção na causa raiz** — conforme os padrões do projeto (camadas, SSR-safety, regra BFF, componentes Material)
4. **Verificar tipagem** — `npx nx affected -t typecheck` após cada correção
5. **Executar os testes existentes** — garantir que nenhum teste quebrou com a mudança

Regras durante a implementação:

- Respeitar a regra BFF: **nenhuma correção pode introduzir chamada direta do browser ao backend interno**
- Respeitar as camadas, boundaries e regras de dependência de [[angular-clean-architecture]]
...
|------|-------------|-------------|
| Teste unitário | Bug em lógica isolada (entidade, use case, componente, pipe) | Vitest + Angular Testing Library; stubs pelos tokens dos contratos |
| Teste de integração | Bug na comunicação entre componentes/camadas (formulário + BFF, repository + HTTP) | Vitest + MSW interceptando os contratos de API |
| Teste E2E | Bug visível na interface, no fluxo completo do usuário ou no SSR (conteúdo do servidor) | Playwright via [[playwright-generate-test]] |
...
**5.1 Validação técnica** — execute via Nx e **não prossiga com falhas**:
...
4. Verifique `browser_console_messages` — sem erros novos (especialmente NG05xx de hydration)
...
## Validação
- `npx nx affected -t typecheck`: SEM ERROS
- `npx nx affected -t lint`: LIMPO
- `npx nx affected -t test`: TODOS PASSANDO
- `npx nx affected -t build`: OK
...
1. **Commit por bugfix** no padrão [[conventional-commit]]:
   - `fix(scope): description` — o scope é o domínio/contexto da mudança (ex.: `billing`, `auth`, `ui`)
   - Exemplo: `fix(billing): show server validation errors on invoice form`
...
7. **Regra BFF intocável** — nenhuma correção introduz chamada direta do browser ao backend interno
...
| `ngSkipHydration` para "resolver" hydration | Eliminar o render não-determinístico que causa o mismatch |
| `::ng-deep`/`!important`/largura fixa para "resolver" layout | Corrigir a estratégia de layout/tema (tokens, mobile-first, grid/flex corretos) |
| Mover chamada para o browser para "simplificar" | Manter a integração via SSR/BFF — corrigir a camada servidor |
| `setTimeout` para "esperar" o estado atualizar (zoneless) | Escrever o estado em signal; derivar com computed |
...
npx nx test [projeto] -- --run [nome-do-teste-ou-arquivo]
...
git add <files>
git commit -m "fix(billing): show server validation errors on invoice form"
```
...
- Causas raiz recorrentes (ex.: hydration por render não-determinístico, auth em localStorage, estado mutado fora de signal, contrato de API divergente)
- Componentes mais propensos a bugs e seus modos de falha
- Lacunas sistemáticas de teste que deixam bugs passarem (edge cases, breakpoints não cobertos, cenários SSR)
