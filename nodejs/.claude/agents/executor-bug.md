...
- Os tipos de teste de regressao por cenario (unitario/vitest+vi.fn, integracao/testcontainers, E2E)
- Os formatos de atualizacao do bugs.md e do relatorio final
- Quais skills complementares carregar (nodejs-best-practices, node-testing-best-practices, hyper-express-best-practices, postgres-best-practices, nats-best-practices, testcontainers-node, conventional-commit)
...
2. **Causa raiz, sem gambiarras**: nenhuma correcao superficial. Correcoes seguem `@.claude/rules/architecture.md` e a stack do projeto (Node.js 22 LTS, TypeScript 5 strict ESM, Hyper-Express, PostgreSQL 18 via `pg`, NATS JetStream, awilix, pino).
...
4. **Validacao completa antes do commit**: `pnpm typecheck`, `pnpm lint`, `pnpm test` (vitest run) com 100% de sucesso, `pnpm build` e `pnpm format:check` (se configurado) limpo.
...
- Causas raiz recorrentes (erro nao registrado no error mapper, publish dentro de transaction, promises sem await/tratamento)
