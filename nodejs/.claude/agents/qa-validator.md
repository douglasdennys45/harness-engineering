...
- Quais skills complementares carregar (nodejs-best-practices, node-testing-best-practices, hyper-express-best-practices, postgres-best-practices, nats-best-practices, testcontainers-node)
...
6. **Padroes do projeto**: valide contra `@.claude/rules/architecture.md`. Stack: Node.js 22 LTS, TypeScript 5 (strict, ESM), Hyper-Express, PostgreSQL 18 (`pg`), NATS JetStream, awilix, pino, swagger-jsdoc. Ambiente: localhost (porta padrao 3000); contrato OpenAPI gerado via swagger-jsdoc (ex: `docs/openapi.json`).
