...
description: Aplica melhores práticas de Bugfix para correção sistemática de bugs documentados pelo QA em serviços Node.js (TypeScript). Cobre leitura do arquivo `.cognup/specs/[feature]/report/bugs.md`, análise de causa raiz (nunca sintomas), planejamento por bug, implementação da correção conforme Clean Architecture (`.claude/rules/architecture.md`) e stack do projeto (Hyper-Express, PostgreSQL 18, NATS JetStream, awilix, pino, zod), criação de testes de regressão (unitário com vitest/vi.fn, integração com testcontainers, E2E de API), validação completa (`pnpm typecheck`, `pnpm lint`, `pnpm format:check`, `pnpm test`, `pnpm build`), atualização do bugs.md com status das correções, relatório final, commits no padrão Conventional Commits e ciclo de revisão (task-reviewer até APPROVED, depois qa-validator até APROVADO). Use sempre que houver bugs documentados em bugs.md para corrigir, quando o QA reprovar uma feature, ao corrigir regressões, ou ao criar testes de regressão para bugs encontrados.
---

# Bugfix Best Practices — Correção Sistemática de Bugs Node.js

Skill que aplica o processo metódico de **Bugfix** sobre bugs documentados pelo QA (`qa-best-practices`) em serviços Node.js (TypeScript). Combina análise de causa raiz, correção conforme os padrões do projeto, testes de regressão, validação completa, rastreabilidade no `bugs.md` e o ciclo de revisão até aprovação.

Combina com [[nodejs-best-practices]] (padrões de código TypeScript moderno) e [[node-testing-best-practices]] (testes de regressão), e — conforme o componente afetado — com [[hyper-express-best-practices]] (HTTP), [[postgres-best-practices]] (banco), [[nats-best-practices]] (mensageria) e [[testcontainers-node]] (integração). Commits seguem [[conventional-commit]]. A arquitetura de referência é `.claude/rules/architecture.md` (monorepo pnpm workspaces com Hyper-Express, PostgreSQL 18, NATS JetStream e awilix).

## Princípio Fundamental

> **Causa raiz, nunca sintoma.** Todo bug é corrigido na origem do problema — sem gambiarras, sem supressão de sintomas, sem `if` defensivo escondendo o defeito. Cada correção vem acompanhada de testes de regressão que falhariam se a correção fosse revertida. O bugfix só termina quando TODOS os bugs do `bugs.md` estão corrigidos, TODOS os testes passam e o ciclo de revisão retorna APPROVED.
...
- **Nada finaliza com teste falhando** — `pnpm test` (vitest run) com 100% de sucesso é pré-condição
...
- Carregar [[nodejs-best-practices]] (sempre) e as skills de domínio conforme os componentes afetados: [[hyper-express-best-practices]] para bugs em controllers/handlers, [[postgres-best-practices]] para bugs em repositórios/queries/migrations, [[nats-best-practices]] para bugs em publishers/subscribers
...
|---|---|
| Endpoint retorna 500 | Erro não mapeado no `ErrorMapper` ou erro não tratado no use case |
| Dado não persiste | Transaction sem commit, `RETURNING` ausente, rollback silencioso, promise de escrita não aguardada |
| Evento não chega ao consumer | Publish antes do commit, subject errado, falta de `Nats-Msg-Id` |
| Teste flakey | Promise não aguardada, dependência de ordem, estado compartilhado entre testes, timer real onde caberia fake timer |
| Crash intermitente | Unhandled rejection, acesso a `undefined` em caminho de erro não coberto, `PoolClient` sem release esgotando o pool |
...
3. **Implementar a correção na causa raiz** — conforme os padrões do projeto (camadas, interfaces de domínio, error handling com classes de erro e `cause`, eventos pós-commit)
4. **Verificar tipos** — `pnpm typecheck` após cada correção
5. **Executar os testes existentes** — garantir que nenhum teste quebrou com a mudança

Regras durante a implementação:

- Respeitar a regra de dependência das camadas (`domain` → `application` → `infrastructure`)
...
- **Cubram edge cases relacionados** — variações do mesmo problema (null/undefined, vazio, limites, concorrência)

> Referência: [[node-testing-best-practices]] (test.each, vi.fn, fixtures) e [[testcontainers-node]] (serviços reais).

| Tipo | Quando Usar | Ferramentas |
|------|-------------|-------------|
| Teste unitário | Bug em lógica isolada de função/método/use case | vitest + `test.each` + `vi.fn()` sobre interfaces de domínio |
| Teste de integração | Bug na comunicação entre módulos ou com infra (repository, publisher/subscriber) | testcontainers (PostgreSQL/NATS reais) |
| Teste E2E | Bug visível no fluxo completo da API | servidor Hyper-Express em porta efêmera + fetch / curl contra a API |

Nomeie os testes de forma rastreável ao bug (ex.: `it("BUG-03: duplicate email returns 409", ...)` ou `describe("createOrder regression BUG-03", ...)`).
...
pnpm lint                 # eslint
pnpm format:check         # prettier --check — nenhum arquivo listado
pnpm test                 # vitest run — 100% de sucesso
pnpm build                # tsc
```
...
7. **Validação completa antes do commit** — typecheck, lint, format:check, test, build
...
12. **Use o Context7 MCP apenas quando** as skills e regras locais não cobrirem a dúvida (API de lib externa, comportamento de versão específica)

## Anti-Patterns de Bugfix (Evitar)

| Anti-Pattern | Correto |
|---|---|
| Silenciar o sintoma (`catch { return null }`) | Corrigir a causa raiz do erro |
| `try/catch` amplo para esconder crash | Eliminar o acesso a `undefined` / estado inválido que causa o crash |
...
| Marcar "Corrigido" sem rodar a suite completa | `pnpm test` (vitest run) a 100% antes de atualizar o bugs.md |
...
| Retry/sleep para "resolver" teste flakey | Corrigir a promise não aguardada ou o estado compartilhado; usar fake timers e espera por condição |
...
pnpm vitest run apps/<app>/src/application/usecase/order/create-usecase.spec.ts -t "BUG-03"
...
# Cobertura dos diretórios corrigidos
pnpm vitest run --coverage apps/<app>/src
...
- Causas raiz recorrentes (ex.: erro de domínio não registrado no ErrorMapper, publish dentro de transaction, promise não aguardada)
- Componentes mais propensos a bugs e seus modos de falha
- Lacunas sistemáticas de teste que deixam bugs passarem (edge cases não cobertos)
