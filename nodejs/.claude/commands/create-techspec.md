...
- CONSULTE AS SKILLS TECNICAS (brainstorming, nodejs-best-practices, node-testing-best-practices, testcontainers-node, e as skills de dominio aplicaveis: hyper-express-best-practices, postgres-best-practices, nats-best-practices) ANTES de propor abordagens.
...
- **Node.js Best Practices** (`@.claude/skills/nodejs-best-practices/SKILL.md`): embasa decisoes arquiteturais na etapa 0 e no design da spec — error handling, tipagem estrita, async/await, ESM, design de interfaces e de APIs.
- **Node Testing Best Practices** (`@.claude/skills/node-testing-best-practices/SKILL.md`): define a estrategia de testes desde o discovery — piramide completa com vitest, mocks com `vi.fn`/`vi.mock`, testes de aceitacao e E2E com testcontainers.
- **Testcontainers Node** (`@.claude/skills/testcontainers-node/SKILL.md`): planeja testes de integracao com servicos reais (PostgreSQL, NATS) em vez de mocks — modulos, wait strategies, cleanup patterns.

**Consultadas conforme o escopo da feature:**

- **Hyper-Express Best Practices** (`@.claude/skills/hyper-express-best-practices/SKILL.md`): se a feature expoe endpoints HTTP — roteamento, request/response, parsing/validacao com zod, error handling centralizado, middlewares, graceful shutdown.
...
- Rules do projeto: `@.claude/rules/architecture.md` — toda decisao deve ser conforme; desvios exigem justificativa e alternativa conforme. Padroes de Hyper-Express, Postgres e NATS vivem nas skills de dominio (hyper-express-best-practices, postgres-best-practices, nats-best-practices)
...
2. **Proponha 2-5 abordagens tecnicas** com pros/contras, **liderando com a recomendacao** e justificativa, validadas contra nodejs-best-practices e as skills de dominio aplicaveis (hyper-express-best-practices para HTTP, postgres-best-practices para banco, nats-best-practices para mensageria).
...
4. **Estrategia de testes desde o inicio** (node-testing-best-practices + testcontainers-node): interfaces que precisarao de mocks (`vi.fn`), cenarios para testes parametrizados com `test.each` no vitest, necessidade de mutation testing (Stryker), quais servicos usam testcontainers em vez de mocks.
...
6. **Resumo de convergencia**: abordagens viaveis com trade-offs, bibliotecas candidatas (com links), padroes Node.js/TypeScript recomendados, estrategia de testes preliminar, riscos tecnicos e pontos de PoC.
...
- Mapear arquivos, modulos, interfaces e pontos de integracao implicados: chamadores/chamados, configs, persistencia, mensageria, concorrencia/async, tratamento de erros, testes, infra
...
- Preencher "Conformidade com Padroes" citando as rules aplicaveis de `@.claude/rules` e as skills de dominio consultadas (hyper-express-best-practices, postgres-best-practices, nats-best-practices)
...
- [ ] Skills consultadas antes de propor abordagens (brainstorming, nodejs-best-practices, node-testing-best-practices, testcontainers-node + skills de dominio aplicaveis: hyper-express-best-practices, postgres-best-practices, nats-best-practices)
