---
description: Executa a proxima tarefa disponivel da feature - implementacao Node.js completa com testes, review, commit e QA
argument-hint: [num-nome-funcionalidade] [numero-da-task (opcional)]
---

Voce e um assistente IA responsavel por implementar tarefas de forma correta e completa em um servico Node.js (TypeScript). Identifique a proxima tarefa disponivel, prepare o contexto e **IMPLEMENTE** ate a conclusao.

## Regras Inegociaveis

<critical>
1. A tarefa so esta completa quando **todos os testes passam com 100% de sucesso** (`pnpm test` — vitest run, sem watch).
...
|-------|----------------|
| `nodejs-best-practices` | Qualquer codigo TypeScript/Node.js: error handling, async/await, tipagem estrita, ESM, nomenclatura |
| `node-testing-best-practices` | Testes: vitest, `vi.fn`/`vi.mock`, `test.each`, cobertura, mutation testing (Stryker) |
| `hyper-express-best-practices` | Controllers, rotas, middlewares, parsing/validacao HTTP com zod (camada infrastructure/controller) |
...
| `testcontainers-node` | Testes de integracao com PostgreSQL/NATS reais |
| `conventional-commit` | Mensagem de commit ao finalizar cada tarefa |

Regras durante a implementacao:

- Siga rigorosamente `@.claude/rules/architecture.md`: Clean Architecture, regra de dependencia entre camadas, nomenclatura kebab-case, fabricas retornando interface de dominio, composicao awilix em `src/bin/api.ts` / `src/bin/consumer.ts`.
...
Se existir script de formatacao configurado no projeto (Prettier), execute tambem `pnpm format:check`.
...
3. Gere a mensagem com a skill `conventional-commit`:
   ...
   - Exemplos: `feat(order): add create order use case and repository`, `test(order): add parameterized tests for order validation`
...
- [ ] `pnpm typecheck`, `pnpm lint`, `pnpm test` (vitest run) e `pnpm build` passando 100%
