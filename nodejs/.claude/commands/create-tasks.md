...
- CONSULTE AS SKILLS TECNICAS (brainstorming, nodejs-best-practices, node-testing-best-practices, testcontainers-node) ANTES de propor a divisao de tarefas.
</critical>
...
- **Node.js Best Practices** (`@.claude/skills/nodejs-best-practices/SKILL.md`): embasa o sequenciamento — camadas da clean architecture (domain -> application -> infrastructure -> bin), padroes de composicao com awilix, design de interfaces e error handling.
- **Node Testing Best Practices** (`@.claude/skills/node-testing-best-practices/SKILL.md`): define os testes de cada tarefa — testes parametrizados com `test.each`, vitest, `vi.fn`/`vi.mock`, cobertura, mutation testing (Stryker).
- **Testcontainers Node** (`@.claude/skills/testcontainers-node/SKILL.md`): planeja testes de integracao com servicos reais (PostgreSQL, NATS) em vez de mocks — modulos pre-configurados, wait strategies, cleanup patterns.
...
- **Rules do projeto:** `@.claude/rules/architecture.md` — toda decomposicao deve ser conforme; desvios exigem justificativa. Padroes de Hyper-Express, Postgres e NATS vivem nas skills de dominio (hyper-express-best-practices, postgres-best-practices, nats-best-practices)
...
3. **Mapeie os artefatos por camada** (conforme architecture.md):
   ...
   - **Infrastructure**: repositories (PostgreSQL via `pg`), publishers (NATS JetStream), controllers (Hyper-Express, se API), subscribers (NATS, se Consumer), adapters, config
   - **Migrations**: arquivos SQL (dbmate, secoes `-- migrate:up`/`-- migrate:down`) em `migrations/<app>/` conforme a skill `postgres-best-practices`
   - **Composicao awilix**: quais binarios sao afetados (API, Consumer, ambos?) e o wiring do container em `src/bin/api.ts` / `src/bin/consumer.ts`
4. **Antecipe a estrategia de testes por camada** (node-testing-best-practices + testcontainers-node):
   - **Domain**: testes parametrizados (`test.each`) com vitest para construtores e validacoes
   - **Application**: use cases com mocks de repository/event via `vi.fn`
   - **Infrastructure**: integracao com testcontainers usando servicos reais — PostgreSQL (modulo `postgres`) para repositories, NATS (container `nats`) para publishers/subscribers; controllers via requests HTTP reais contra a instancia Hyper-Express de teste
...
3. **Infrastructure em seguida**: repositories (PostgreSQL via `pg`), publishers (NATS JetStream), controllers (Hyper-Express), subscribers, adapters — implementam interfaces de domain
...
5. **Composicao awilix por ultimo**: wiring do container no binario (`src/bin/api.ts` e/ou `src/bin/consumer.ts`)
...
- OpenAPI (geracao via swagger-jsdoc) vem apos os controllers estarem prontos (apenas APIs)
...
### Testes de Unidade
- **Entidades de dominio**: testes parametrizados (`test.each`) para construtores e validacoes
- **Use cases**: mocks de interfaces (repository, event) com `vi.fn`; testar orquestracao e propagacao de erros
- **Controllers**: parsing de request, validacao de input (schemas zod), mapeamento de erros de dominio -> HTTP status
...
- **Padroes**: modulos pre-configurados sempre que disponiveis, cleanup dos containers em `afterAll`/hooks de teardown, wait strategies adequadas

### Ferramentas
- **Assertions**: vitest (`expect`; use `expect(...).rejects`/`resolves` para async)
- **Mocking**: `vi.fn`/`vi.mock` para interfaces de dominio
- **HTTP**: requests reais (fetch/undici) contra a instancia Hyper-Express de teste
- **Mutation testing**: Stryker para validar a qualidade dos testes
...
- [ ] Skills consultadas antes de propor a divisao (brainstorming, nodejs-best-practices, node-testing-best-practices, testcontainers-node)
- [ ] PRD e Tech Spec lidos; secoes de Sequenciamento e Testes da Tech Spec aproveitadas
- [ ] Codebase explorado e artefatos mapeados por camada (domain, application, infrastructure, migrations, awilix)
...
- [ ] Migrations SQL e composicao awilix contempladas nas tarefas (quando aplicavel)
