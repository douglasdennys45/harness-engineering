# Harness de Engenharia — Rust

Este diretório contém um **harness de desenvolvimento orientado a specs** para serviços Rust usando Claude Code. Ele define um pipeline completo — da ideia ao merge — em que cada fase produz um artefato versionado, e a implementação só avança com aprovação explícita (do usuário nas fases de planejamento, dos agents de review/QA na fase de execução).
...
| `skills/` | Boas práticas consultadas pelos commands e agents (Rust, Axum, PostgreSQL, NATS, testes, commits, QA, review, bugfix, brainstorming, PRD) |
...
Brainstorm técnico sobre o PRD: lê o codebase, propõe 2-5 abordagens arquiteturais validadas contra as skills de domínio (Rust, Axum, Postgres, NATS) e as rules do projeto, avalia reusar vs construir, e define a estratégia de testes desde o início. Só gera a spec após o usuário confirmar a direção técnica.
...
Lê PRD + Tech Spec + codebase, mapeia os artefatos por camada (domain → application → infrastructure → migrations → composition root) e propõe 2-3 abordagens de decomposição. **Mostra a lista high-level e aguarda aprovação explícita** antes de gerar qualquer arquivo.
...
3. **Implementação:** segue `rules/architecture.md` e consulta as skills conforme o contexto (Axum para controllers, Postgres para repositórios, NATS para mensageria, etc.). Testes escritos junto com o código.
4. **Validação (gate):** `cargo build --workspace`, `cargo clippy --workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check`, `cargo test --workspace` — 100% de sucesso é pré-condição.
...
|---|---|---|---|
| `task-reviewer` | Após cada tarefa commitada | Revisa o diff contra idiomas Rust, Clean Architecture e a stack; roda as verificações automatizadas; grava `tasks/reviews/[num]_task_review.md` | `APPROVED` \| `APPROVED WITH OBSERVATIONS` \| `CHANGES REQUESTED` |
| `qa-validator` | Quando todas as tasks estão APPROVED | Pipeline de QA completo: análise estática, testes, mutation testing, E2E de API, contrato OpenAPI, auditoria de código e infra. **Tolerância zero:** qualquer bug = REPROVADO | `APROVADO` \| `REPROVADO` |
...
| Skill | Quando |
|---|---|
| `rust-best-practices` | Qualquer código Rust |
| `rust-testing-best-practices` | Testes (mockall, #[tokio::test], rstest, integração) |
| `testcontainers-rust` | Testes de integração com Postgres/NATS reais |
| `axum-best-practices` | Controllers, rotas, middlewares HTTP |
...
- **Rust (edition 2024)** em monorepo (Cargo workspace) com N apps independentes (`apps/<app>/`) + crate compartilhado (`pkg/`)
- **Clean Architecture** por app: `domain/` (entidades + traits) → `application/` (use cases) → `infrastructure/` (controllers, repos, publishers, subscribers) → `src/bin/` (composition root)
- **Axum 0.8** (HTTP), **PostgreSQL 18** (sqlx, SQL manual, sem ORM), **NATS JetStream 2.12** (mensageria entre apps via async-nats), **composition root manual + `Arc<dyn Trait>`** (DI), **tracing** (logging estruturado JSON)
- 1 use case = 1 trait = 1 arquivo, com método único `perform`
...
3. Tarefa só está completa com testes 100% verdes (`cargo test --workspace` + clippy sem warnings) **e** review APPROVED.
