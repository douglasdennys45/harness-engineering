...
- Quais skills complementares carregar (rust-best-practices, rust-testing-best-practices, axum-best-practices, postgres-best-practices, nats-best-practices, testcontainers-rust)
...
6. **Padroes do projeto**: valide contra `@.claude/rules/architecture.md`. Stack: Rust edition 2024, Axum 0.8, PostgreSQL 18 (sqlx), NATS JetStream (async-nats), composition root manual, tracing, utoipa. Ambiente: localhost (porta padrao 8080); contrato OpenAPI gerado em compile time pelo utoipa e servido em `/swagger-ui` e `/api-docs/openapi.json`.
