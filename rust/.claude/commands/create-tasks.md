...
- CONSULTE AS SKILLS TECNICAS (brainstorming, rust-best-practices, rust-testing-best-practices, testcontainers-rust) ANTES de propor a divisao de tarefas.
</critical>
...
- **Rust Best Practices** (`@.claude/skills/rust-best-practices/SKILL.md`): embasa o sequenciamento — camadas da clean architecture (domain -> application -> infrastructure -> src/bin), composition root manual com `Arc<dyn Trait>`, design de traits e error handling.
- **Rust Testing Best Practices** (`@.claude/skills/rust-testing-best-practices/SKILL.md`): define os testes de cada tarefa — testes table-driven, asserts nativos com `#[tokio::test]`, mockall, benchmarks, data races prevenidas em compile time pelo ownership.
- **Testcontainers Rust** (`@.claude/skills/testcontainers-rust/SKILL.md`): planeja testes de integracao com servicos reais (PostgreSQL, NATS) em vez de mocks — modulos pre-configurados, wait strategies, cleanup patterns.
...
- **Rules do projeto:** `@.claude/rules/architecture.md` — toda decomposicao deve ser conforme; desvios exigem justificativa. Padroes de Axum, Postgres e NATS vivem nas skills de dominio (axum-best-practices, postgres-best-practices, nats-best-practices)
...
3. **Mapeie os artefatos por camada** (conforme architecture.md):
   - **Domain**: entidades, traits de use case (1 acao = 1 trait = 1 arquivo), repository, event, traits de libs externas
   - **Application**: implementacoes de use case (dependem apenas de domain)
   - **Infrastructure**: repositories (PostgreSQL via sqlx), publishers (NATS JetStream), controllers (Axum, se API), subscribers (NATS, se Consumer), adapters, config
   - **Migrations**: arquivos `up.sql`/`down.sql` (sqlx-cli, `sqlx migrate add -r`) em `migrations/<app>/` conforme a skill `postgres-best-practices`
   - **Composition root**: quais binarios sao afetados (API, Consumer, ambos?) e o wiring manual em `src/bin/api.rs` / `src/bin/consumer.rs`
4. **Antecipe a estrategia de testes por camada** (rust-testing-best-practices + testcontainers-rust):
   - **Domain**: testes table-driven com asserts nativos para construtores e validacoes
   - **Application**: use cases com mocks de repository/event via mockall
   - **Infrastructure**: integracao com testcontainers usando servicos reais — PostgreSQL (modulo `postgres`) para repositories, NATS (modulo `nats`) para publishers/subscribers; controllers via `tower::ServiceExt::oneshot` sobre o `Router` de teste
   - Identifique traits que precisarao de mocks e os cenarios criticos de negocio
...
- As **dependencias** respeitam a ordem da clean architecture (domain -> application -> infrastructure -> src/bin)?
...
2. **Tarefas individuais** (`tasks/[num]_task.md`): template `@.claude/templates/task-template.md` — para cada tarefa detalhar:
   ...
   - Estrategia de testes especifica da tarefa (traits a mockar, cenarios happy path + error cases, tipo de teste, ferramentas)
...
1. **Domain primeiro**: entidades e traits (use case, repository, event, libs externas) — sem dependencias de infraestrutura
2. **Application depois**: implementacao dos use cases — depende apenas de domain
3. **Infrastructure em seguida**: repositories (PostgreSQL via sqlx), publishers (NATS JetStream), controllers (Axum), subscribers, adapters — implementam traits de domain
4. **Migrations SQL**: junto da tarefa de repository de infraestrutura (sqlx-cli, `.up.sql`/`.down.sql`, conforme postgres-best-practices)
5. **Composition root por ultimo**: wiring manual no binario (`src/bin/api.rs` e/ou `src/bin/consumer.rs`)
...
- OpenAPI (utoipa, gerado em compile time via `#[utoipa::path]` — sem passo de CLI) vem apos os controllers estarem prontos (apenas APIs)

## Diretrizes de Estrategia de Testes por Tarefa

Cada tarefa deve especificar: quais **traits** precisam de mocks, quais **cenarios** (happy path + error cases) cobrir, qual **tipo de teste** por subtarefa e quais **ferramentas** usar.

### Testes de Unidade
- **Entidades de dominio**: testes table-driven para construtores e validacoes
- **Use cases**: mocks de traits (repository, event) com mockall; testar orquestracao e propagacao de erros
- **Controllers**: parsing de request, validacao de input (derive `Validate`), mapeamento de erros de dominio -> HTTP status
...
- **Padroes**: modulos pre-configurados sempre que disponiveis (testcontainers-modules), `AsyncRunner` com cleanup automatico no drop do container, wait strategies adequadas

### Ferramentas
- **Assertions**: asserts nativos (`assert!`, `assert_eq!`; use `matches!` para variantes de erro) com `#[tokio::test]` para codigo async
- **Mocking**: mockall para traits de dominio (`#[cfg_attr(test, mockall::automock)]`)
- **HTTP**: `tower::ServiceExt::oneshot` sobre o `Router` de teste; E2E completo com `axum::serve` + reqwest
- **Data races**: prevenidas em compile time pelo ownership/borrow checker — sem race detector a executar
...
- [ ] Skills consultadas antes de propor a divisao (brainstorming, rust-best-practices, rust-testing-best-practices, testcontainers-rust)
- [ ] PRD e Tech Spec lidos; secoes de Sequenciamento e Testes da Tech Spec aproveitadas
- [ ] Codebase explorado e artefatos mapeados por camada (domain, application, infrastructure, migrations, composition root)
...
- [ ] Migrations SQL e composition root contemplados nas tarefas (quando aplicavel)
