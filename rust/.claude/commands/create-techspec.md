...
- CONSULTE AS SKILLS TECNICAS (brainstorming, rust-best-practices, rust-testing-best-practices, testcontainers-rust, e as skills de dominio aplicaveis: axum-best-practices, postgres-best-practices, nats-best-practices) ANTES de propor abordagens.
...
- **Rust Best Practices** (`@.claude/skills/rust-best-practices/SKILL.md`): embasa decisoes arquiteturais na etapa 0 e no design da spec — error handling (thiserror/anyhow), ownership e borrowing, traits, concorrencia async com Tokio, design de APIs.
- **Rust Testing Best Practices** (`@.claude/skills/rust-testing-best-practices/SKILL.md`): define a estrategia de testes desde o discovery — piramide completa com testes unitarios em `#[cfg(test)]`, mocks com mockall, testes de aceitacao e E2E com testcontainers.
- **Testcontainers Rust** (`@.claude/skills/testcontainers-rust/SKILL.md`): planeja testes de integracao com servicos reais (PostgreSQL, NATS) em vez de mocks — modulos, wait strategies, cleanup patterns.

**Consultadas conforme o escopo da feature:**

- **Axum Best Practices** (`@.claude/skills/axum-best-practices/SKILL.md`): se a feature expoe endpoints HTTP — roteamento, extractors, parsing/validacao com `ValidatedJson`, error handling centralizado (`IntoResponse`), middlewares tower, graceful shutdown.
...
- Rules do projeto: `@.claude/rules/architecture.md` — toda decisao deve ser conforme; desvios exigem justificativa e alternativa conforme. Padroes de Axum, Postgres e NATS vivem nas skills de dominio (axum-best-practices, postgres-best-practices, nats-best-practices)
...
2. **Proponha 2-5 abordagens tecnicas** com pros/contras, **liderando com a recomendacao** e justificativa, validadas contra rust-best-practices e as skills de dominio aplicaveis (axum-best-practices para HTTP, postgres-best-practices para banco, nats-best-practices para mensageria).
...
4. **Estrategia de testes desde o inicio** (rust-testing-best-practices + testcontainers-rust): traits que precisarao de mocks (mockall), cenarios para testes table-driven com asserts nativos, necessidade de benchmarks — data races sao prevenidas em compile time pelo ownership/borrow checker —, quais servicos usam testcontainers em vez de mocks.
5. **Perguntas provocativas de arquitetura** (uma por vez):
   - "Existe [crate Z] que resolve 80% disso. Usamos ou preferimos custom?"
   - "Se precisasse escalar 10x, essa abordagem ainda funcionaria?"
   - "Para testar esse componente, o trait atual permite mock adequado?"
6. **Resumo de convergencia**: abordagens viaveis com trade-offs, bibliotecas candidatas (com links), padroes Rust recomendados, estrategia de testes preliminar, riscos tecnicos e pontos de PoC.
...
- Mapear arquivos, modulos, traits e pontos de integracao implicados: chamadores/chamados, configs, persistencia, mensageria, concorrencia/async, tratamento de erros, testes, infra
...
- Preencher "Conformidade com Padroes" citando as rules aplicaveis de `@.claude/rules` e as skills de dominio consultadas (axum-best-practices, postgres-best-practices, nats-best-practices)
...
- [ ] Skills consultadas antes de propor abordagens (brainstorming, rust-best-practices, rust-testing-best-practices, testcontainers-rust + skills de dominio aplicaveis: axum-best-practices, postgres-best-practices, nats-best-practices)
