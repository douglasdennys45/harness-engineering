...
- CONSULTE AS SKILLS TECNICAS (brainstorming, python-best-practices, python-testing-best-practices, testcontainers-python, e as skills de dominio aplicaveis: robyn-best-practices, postgres-best-practices, nats-best-practices) ANTES de propor abordagens.
...
- **Python Best Practices** (`@.claude/skills/python-best-practices/SKILL.md`): embasa decisoes arquiteturais na etapa 0 e no design da spec — type hints estritos, async/await, tratamento de erros, design de Protocols e de APIs.
- **Python Testing Best Practices** (`@.claude/skills/python-testing-best-practices/SKILL.md`): define a estrategia de testes desde o discovery — piramide completa com pytest, fakes/mocks, testes de aceitacao e E2E com testcontainers.
- **Testcontainers Python** (`@.claude/skills/testcontainers-python/SKILL.md`): planeja testes de integracao com servicos reais (PostgreSQL, NATS) em vez de mocks — modulos, wait strategies, cleanup patterns.

**Consultadas conforme o escopo da feature:**

- **Robyn Best Practices** (`@.claude/skills/robyn-best-practices/SKILL.md`): se a feature expoe endpoints HTTP — roteamento, request/response, validacao com Pydantic, error handling centralizado, middlewares, OpenAPI, graceful shutdown.
...
- Rules do projeto: `@.claude/rules/architecture.md` — toda decisao deve ser conforme; desvios exigem justificativa e alternativa conforme. Padroes de Robyn, Postgres e NATS vivem nas skills de dominio (robyn-best-practices, postgres-best-practices, nats-best-practices)
...
2. **Proponha 2-5 abordagens tecnicas** com pros/contras, **liderando com a recomendacao** e justificativa, validadas contra python-best-practices e as skills de dominio aplicaveis (robyn-best-practices para HTTP, postgres-best-practices para banco, nats-best-practices para mensageria).
...
4. **Estrategia de testes desde o inicio** (python-testing-best-practices + testcontainers-python): interfaces (Protocols) que precisarao de fakes/mocks, cenarios para testes parametrizados com `pytest.mark.parametrize`, necessidade de mutation testing (mutmut), quais servicos usam testcontainers em vez de mocks.
5. **Perguntas provocativas de arquitetura** (uma por vez):
   ...
   - "Para testar esse componente, o Protocol atual permite fake/mock adequado?"
6. **Resumo de convergencia**: abordagens viaveis com trade-offs, bibliotecas candidatas (com links), padroes Python recomendados, estrategia de testes preliminar, riscos tecnicos e pontos de PoC.
...
- Mapear arquivos, modulos, Protocols e pontos de integracao implicados: chamadores/chamados, configs, persistencia, mensageria, concorrencia/async, tratamento de erros, testes, infra
...
- **Interfaces principais**: contratos (Protocols) e responsabilidades dos componentes
...
- Preencher "Conformidade com Padroes" citando as rules aplicaveis de `@.claude/rules` e as skills de dominio consultadas (robyn-best-practices, postgres-best-practices, nats-best-practices)
...
- [ ] Skills consultadas antes de propor abordagens (brainstorming, python-best-practices, python-testing-best-practices, testcontainers-python + skills de dominio aplicaveis: robyn-best-practices, postgres-best-practices, nats-best-practices)
...
- [ ] Estrategia de testes definida (tipos, ferramentas de fake/mock, testcontainers, cenarios criticos)
