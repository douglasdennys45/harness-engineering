...
```python
# Exemplo de definição de interface (Protocol)
class NomeUseCase(Protocol):
    async def perform(self, input: Entrada) -> Saida: ...
```
...
- Fluxos completos de API a testar (request → processamento → response)
- Cenários com múltiplos serviços envolvidos
