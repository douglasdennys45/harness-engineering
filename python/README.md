# Harness de Engenharia — Python

Este diretório contém um **harness de desenvolvimento orientado a specs** para serviços Python usando Claude Code. Ele define um pipeline completo — da ideia ao merge — em que cada fase produz um artefato versionado, e a implementação só avança com aprovação explícita (do usuário nas fases de planejamento, dos agents de review/QA na fase de execução).
...
| `skills/` | Boas práticas consultadas pelos commands e agents (Python, Robyn, PostgreSQL, NATS, testes, commits, QA, review, bugfix, brainstorming, PRD) |
...
Brainstorm técnico sobre o PRD: lê o codebase, propõe 2-5 abordagens arquiteturais validadas contra as skills de domínio (Python, Robyn, Postgres, NATS) e as rules do projeto, avalia reusar vs construir, e define a estratégia de testes desde o início. Só gera a spec após o usuário confirmar a direção técnica.
...
Lê PRD + Tech Spec + codebase, mapeia os artefatos por camada (domain → application → infrastructure → migrations → composição dependency-injector) e propõe 2-3 abordagens de decomposição. **Mostra a lista high-level e aguarda aprovação explícita** antes de gerar qualquer arquivo.
...
3. **Implementação:** segue `rules/architecture.md` e consulta as skills conforme o contexto (Robyn para controllers, Postgres para repositórios, NATS para mensageria, etc.). Testes escritos junto com o código.
4. **Validação (gate):** `make typecheck` (mypy --strict), `make lint` (ruff check), `make format-check` (ruff format --check) e `make test` (pytest) — 100% de sucesso é pré-condição.
...
|---|---|---|---|
| `task-reviewer` | Após cada tarefa commitada | Revisa o diff contra os idiomas Python (mypy strict), Clean Architecture e a stack; roda as verificações automatizadas; grava `tasks/reviews/[num]_task_review.md` | `APPROVED` \| `APPROVED WITH OBSERVATIONS` \| `CHANGES REQUESTED` |
| `qa-validator` | Quando todas as tasks estão APPROVED | Pipeline de QA completo: análise estática, testes unitários/integração, mutation testing (mutmut), E2E de API, contrato OpenAPI, auditoria de código e infra. **Tolerância zero:** qualquer bug = REPROVADO | `APROVADO` \| `REPROVADO` |
...
| Skill | Quando |
|---|---|
| `python-best-practices` | Qualquer código Python |
| `python-testing-best-practices` | Testes (pytest, parametrize, AsyncMock, mutmut) |
| `testcontainers-python` | Testes de integração com Postgres/NATS reais |
| `robyn-best-practices` | Controllers, rotas, middlewares HTTP |
...
- **Python 3.13+** (type hints estritos com `mypy --strict`, async/await) em monorepo **uv workspaces** com N apps independentes (`apps/<app>/`) + biblioteca compartilhada (`pkg/`, módulo `org_pkg`)
- **Clean Architecture** por app: `src/<app>/domain/` (entidades + Protocols: `entity/`, `usecase/<contexto>/`, `repository/`, `event/`, `<lib>/`) → `src/<app>/application/usecase/<contexto>/` (use cases) → `src/<app>/infrastructure/` (config, controllers, repos, publishers, subscribers, adapters) → `src/<app>/bin/` (composition root dependency-injector: `api.py` e `consumer.py`)
- **Robyn** (HTTP, runtime Rust), **PostgreSQL 18** via `asyncpg` (SQL manual, sem ORM; migrations com **dbmate**), **NATS JetStream** via `nats-py` (mensageria entre apps), **dependency-injector** (DI), **structlog** (logging estruturado), **Pydantic v2** (validação), **OpenAPI nativo do Robyn** (`/docs`, `/openapi.json`)
- **Testes:** pytest + pytest-asyncio + testcontainers (Postgres/NATS reais) + mutmut (mutation testing); **qualidade:** ruff (lint + format) + mypy strict
- 1 use case = 1 Protocol = 1 arquivo, com método único `perform(input)`; arquivos e módulos em snake_case
...
3. Tarefa só está completa com testes 100% verdes (`make test`) **e** review APPROVED.
